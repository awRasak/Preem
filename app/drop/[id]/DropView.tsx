...
export default async function DropView({
  dropId: id,
  initialTrackId,
}: {
  dropId: string;
            ...
            <div className="divide-y divide-line rounded-xl border border-line">
              {tracks.map((track) => (
                <div
                  key={track.id}
                  id={`track-${track.id}`}
                  className={`flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between ${track.id === initialTrackId ? "bg-surface-2/60" : ""}`}>

...
begin
  ...
  --
  -- NOTE: deliberately NOT security definer (see 0021 for rationale).
...
create policy "admin can read all show tickets"
  on public.show_tickets for select
  using (is_admin());-- Track audio change requests: an artist uploads a replacement file for a
...
alter table public.drop_presaves enable row level security;-- Bio Link: artist-customizable link slots (socials, merch, tickets,
